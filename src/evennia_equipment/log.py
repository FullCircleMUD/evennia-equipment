...
"""Logging shim for evennia-equipment.

Every line the library emits goes to its own ``equipment.log``, co-located
with Evennia's other logs under ``settings.LOG_DIR``, so tracing a wear, a
wield or a weight recalculation means reading one file rather than picking
its lines out of the main server log.
...
the server logs use — so an equipment line and a `server.log` line can be
read against each other directly. Adding our own would stamp every line
twice.

Outside an Evennia engine — tests, or any caller where Evennia is not
bootstrapped — ``equipment_log`` is a silent no-op. The import is lazy and
an ImportError is swallowed; the library deliberately does not fall back to
...
_LOG_FILENAME = "equipment.log"
_VALID_LEVELS = ("INFO", "WARN", "ERROR")


def equipment_log(message: str, level: str = "INFO", trace: bool = False) -> None:
    """Emit one line to ``equipment.log``.
